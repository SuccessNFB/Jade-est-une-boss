...
const STATUS_LABEL: Record<string, { label: string; color: string }> = {
  ...
  refunded:     { label: 'Remboursée',   color: 'bg-[#141412] text-gray-500 border-white/[0.1]' },
        ...
        <div className="section-container max-w-3xl py-10">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-[#F5C542]/10 border border-[#F5C542]/20 flex items-center justify-center">
                <span className="font-bold text-[#F5C542] text-lg uppercase">
          ...
          <div className="grid grid-cols-2 gap-3 mb-10">
            {[
              ...
              <Link
                key={label}
                href={href}
                className="flex items-center gap-3 p-4 rounded-2xl border border-white/[0.06] bg-[#141412] hover:border-[#F5C542]/30 hover:bg-[#F5C542]/5 transition-all group"
              >
                <div className="w-9 h-9 rounded-xl bg-[#1A1A17] shadow-sm flex items-center justify-center">
                  <Icon className="w-4 h-4 text-white/70 group-hover:text-[#F5C542] transition-colors" />
                </div>
          ...
          <section>
            <h2 className="font-serif text-xl font-bold text-white/70 mb-4 flex items-center gap-2">
              <Package className="w-5 h-5 text-[#F5C542]" />
              Mes commandes
            </h2>
            ...
            {orders.length === 0 ? (
              <div className="text-center py-16 rounded-2xl border border-dashed border-white/[0.1]">
                ...
                <Link href="/shop" className="text-sm text-[#F5C542] font-semibold hover:underline">
                ...
                {orders.map((order, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="rounded-2xl border border-white/[0.06] p-5 bg-[#1A1A17] shadow-sm"
                    >
          ...
          {tickets.length > 0 && (
            <section className="mt-10">
              ...
              <div className="space-y-3">
                {tickets.map((t) => (
                  <div key={t.id} className="rounded-2xl border border-white/[0.06] p-4 bg-[#1A1A17] shadow-sm flex items-start justify-between gap-4">
